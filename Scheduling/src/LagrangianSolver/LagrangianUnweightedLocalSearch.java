...
//		System.out.println("-------");
		...
		for (int i = 0; i < n - 1; i++) {
			...
			if (currentSolution[i] < nA)
				newSumA += p[currentSolution[i + 1]];
			else
				newSumB += p[currentSolution[i + 1]];

			if (currentSolution[i + 1] < nA)
				newSumA -= p[currentSolution[i]];
			else
				newSumB -= p[currentSolution[i]];
...
//		if(bestUB!=before)
//			System.out.println("before = "+before+ " after = "+bestUB);
